...
参考项目：
- `F:\插件脚本开发\image-compression`：CSS 变量设计 token、暗色卡片、自研控件
- `F:\插件脚本开发\image-compression-tool-master`：Vue3 + Vite、Uploader / Controls / Preview 分区、进度与统计清晰
...
- **Token 复用**：从 `F:\插件脚本开发\image-compression\css\style.css` 抽取 `--primary-*`、`--dark-bg/panel/card`、间距/圆角，落到 `src/ui-next/styles/tokens.css`
